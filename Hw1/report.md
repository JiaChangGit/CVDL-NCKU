...
這份報告會先說明使用到的技術，再逐題深入探討演算法原理、關鍵字、實作 trace，以及程式如何把理論轉成可互動功能。
...
Q2 的目標是在棋盤影像上畫出使用者輸入的英文文字。這不是把文字畫在固定 pixel，而是將字母線段定義成 3D 座標，再透過相機模型投影到影像上。
